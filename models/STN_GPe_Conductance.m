...
pulse_freq = 130;      % Pulse frequency in Hz
pulse_amp = 55;      % Pulse amplitude in μA/cm²
pulse_width = 0.5;     % Pulse width in ms

% Generate time vector
dt = 0.01;             % Time step in ms
t = 0:dt:1000;         % Time vector from 0 to 1000 ms

% Generate biphasic pulse train
I_dbs = generate_biphasic_dbs(t, pulse_freq, pulse_amp, pulse_width);

%% Vary the conductance to achieve sync action
%g_GPe_STN = 0.4; 
g_GPe_STN = 0.002; 
...
for i = 2:nSteps-1
    ...
    % Total injected currents including synaptic inputs and DBS
    I_syn_GPe_to_STN = g_GPe_STN * s_GPe_STN(i) * (V_STN(i) - E_GPe_STN);  % Conductance-based

     Iinj_STN(i) = Ictx(i) + I_syn_GPe_to_STN;
    %Iinj_STN(i) = Ictx(i) + I_syn_GPe_to_STN + I_dbs(i);  %% inject current here
